...
def check_password():
    ...
    with col2:
        ...
        if password == "6802":  # 은희님이 지정하신 비밀번호
...
# ──────────────────────────────────────────────────────────────
# 3. 커스텀 CSS (라이트/다크 모드 완벽 호환 및 JLPT 뱃지)
# ──────────────────────────────────────────────────────────────
st.markdown("""
<style>
...
# ──────────────────────────────────────────────────────────────
# 7. 초강력 프롬프트 (한국어 문법화, JLPT, 뉘앙스 추가)
# ──────────────────────────────────────────────────────────────
SYSTEM_PROMPT = """당신은 J-POP 가사를 완벽하게 뜯어보는 깐깐하고 독한 1타 일본어 강사입니다.
  ...
  "grammar": [
    {
      ...
      "morphology": "형태소 분석",
      "meaning": "한국어 뜻 설명",
      "usage": "어떤 상황/뉘앙스에서 쓰이는지 설명",
      "example": "예문(일본어) — 한국어 해석"
    }
  ],
...
규칙:
- [초강력 지시] vocabulary는 가사에 등장하는 N1~N5 수준의 어휘를 먼지 한 톨까지 샅샅이 찾아내어 무조건 '최소 20개 이상' 추출할 것. 단어가 부족하면 활용형·파생어·관용 표현까지 포함해 20개를 채우세요.
- [초강력 지시] grammar도 조동사, 접속 표현, 종조사, 회화체 축약 등을 폭넓게 분석하여 무조건 '최소 20개 이상' 추출할 것. 가사 길이가 짧더라도 기초 조사까지 집요하게 분석하여 어떻게든 20개라는 목표치를 채울 것.
- [핵심] grammar의 morphology(형태소 분석)는 일본어 원문이 아닌, '동사 미연형 + 접속조사'와 같이 반드시 '한국어 문법 용어'로 풀어서 설명할 것.
- [핵심] grammar의 usage(사용 상황)는 학습자가 언제 이 표현을 써야 하는지 직관적으로 알 수 있도록 구체적인 뉘앙스를 한국어로 설명할 것.
- 모든 설명은 1타 강사처럼 상세하고 친절한 한국어로 작성할 것. JSON 외 다른 텍스트는 절대 출력하지 말 것.
"""
...
if run:
    ...
    with st.spinner("AI가 가사를 샅샅이 분석 중입니다... 🎧 (분량이 많아 최대 1분 소요)"):
        try:
        ...
        for g in grammar_list:
            ...
            <div class="grammar-card">
                <h4 style="margin:0 0 0.6rem 0;">
                    ▸ {g.get('pattern','')} 
                    <span class="jlpt-badge">{g.get('jlpt', '급수 없음')}</span>
                </h4>
                <p><b>🧩 형태소 분석:</b> {g.get('morphology','')}</p>
                <p><b>📖 의미:</b> {g.get('meaning','')}</p>
                <p><b>💡 사용 상황(뉘앙스):</b> {g.get('usage','')}</p>
                <p><b>✍️ 예문:</b> {g.get('example','')}</p>
            </div>
            """, unsafe_allow_html=True)
